...
export default class Pirate extends Component {
    ...
    _handleAcceleration(accelerationData){
        ...
        const xyzAccelaration = Math.sqrt(accelerationData.y^2 + accelerationData.z^2  + accelerationData.x^2);
        ...
        console.log(`${(new Date()).getMinutes()}:${(new Date()).getSeconds()};${xyzAccelaration}`);
    }

    _goUp(){
        console.log('Pirate going up !');
        this.setState({currentY: this.state.currentY-20});
    }

    _goLeft(){
        console.log('Pirate going left !');
        this.setState({currentX: this.state.currentX-20});
    }

    _goRight(){
        console.log('Pirate going right !');
        this.setState({currentX: this.state.currentX+20});
    }

    _goDown(){
        console.log('Pirate going down !');
        this.setState({currentY: this.state.currentY+20});
    }
    ...
    render(){
        return(
            <View style={styles.container}>
                ...
                >
                    <SvgUri width={iconSize} height={iconSize} source={walkingIcon} fill={'#00FFCC'}  /> 
                </View>
                <TouchableOpacity style={[styles.button, styles.up]} onPress={()=>this._goUp()}>
                    <Text style={styles.buttonText}> Up</Text>
                </TouchableOpacity>
                <TouchableOpacity style={[styles.button, styles.left]} onPress={()=>this._goLeft()}>
                    <Text style={styles.buttonText}> Left</Text>
                </TouchableOpacity>
                <TouchableOpacity style={[styles.button, styles.right]} onPress={()=>this._goRight()}>
                    <Text style={styles.buttonText}> Right</Text>
                </TouchableOpacity>
                <TouchableOpacity style={[styles.button, styles.down]} onPress={()=>this._goDown()}>
